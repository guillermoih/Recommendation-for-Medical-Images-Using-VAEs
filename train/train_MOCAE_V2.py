...
def train(config, model, dataset):
    ...
    model.mocae.compile(loss=loss_function,
        ...
        for epoch in range(n_epochs):
            ...
            for batch in range(n_batches):
                ...
                if class_phase:
                    ...
                    history['loss'].append(losses['loss'])
                    history['rec_loss'].append(losses['loss'])
                    # Fill class loss with last value
                    history['class_loss'].append(history['class_loss'][-1])                
